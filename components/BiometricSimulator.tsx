...
import { Fingerprint, Scan, CheckCircle, AlertTriangle, Radio, Activity } from 'lucide-react';

interface BiometricSimulatorProps {
  employees: Employee[];
  onDevicePunch: (employeeId: string) => { status: 'in' | 'out' | 'error', time: string, message: string };
  currentUser: Employee; // Added currentUser to check role
}

const BiometricSimulator: React.FC<BiometricSimulatorProps> = ({ employees, onDevicePunch, currentUser }) => {
  const [selectedId, setSelectedId] = useState<string>('');
  const [logs, setLogs] = useState<{ id: string; msg: string; type: 'success' | 'error'; time: string }[]>([]);
  ...
  const handleScan = () => {
    ...
    setTimeout(() => {
        const result = onDevicePunch(selectedId);
        ...
        setScanning(false);
        // Do NOT reset ID if it's an employee (keep them selected)
        if (!isEmployeeRole) {
             setSelectedId(''); 
        }
    }, 800);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full max-h-[80vh]">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm p-6 flex flex-col">
            ...
            <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-xs text-blue-700 dark:text-blue-300 border border-blue-100 dark:border-blue-900/30">
                <strong>ملاحظة تقنية:</strong> هذه الشاشة تحاكي عمل جهاز البصمة الفعلي. في النسخة النهائية، سيتم استبدال هذه الواجهة بـ API Service تستقبل البيانات من جهاز ZKTeco أو Hikvision عبر الشبكة مباشرة.
            </div>
        </div>
    </div>
  );
